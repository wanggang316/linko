...
#   else: --keychain-profile "${KEYCHAIN_PROFILE:-linko-notary}"
#         (one-time setup: xcrun notarytool store-credentials linko-notary).
#
set -euo pipefail
...
if [ -n "${AC_API_KEY_ID:-}" ]; then
  ...
  key_path="$(mktemp -t linko-notary).p8"
...
# to pre-staple bytes. Rewrite it here so every caller leaves a
# coherent (file, checksum) pair on disk. No-op when no sidecar exists.
